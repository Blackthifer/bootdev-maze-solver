...
class Maze:
    ...
    def break_entrance_and_exit(self, canvas):
        self.entry = Point(0, 0)
        self.cells[self.entry.x][self.entry.y].hastopwall = False
        self.draw_cell(canvas, 0, 0)
        self.exit = Point(self.rows - 1, self.cols - 1)
        self.cells[self.exit.x][self.exit.y].hasbottomwall = False
        self.draw_cell(canvas, self.rows - 1, self.cols - 1)
    
    def break_walls(self, row, col):
